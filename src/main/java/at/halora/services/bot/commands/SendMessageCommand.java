...
public class SendMessageCommand extends BotCommand {
    ...
    public void execute() {
        ...
        if (bot.getLogic().getUser(receiver) == null) {
            bot.sendBotMessage(userId, "Failed to send message, user '" + receiver + "' does not exist.");
            return;
        }

        //Send message to messagelogic
        bot.getLogic().sendMessage(new Message(LocalTime.now().toString(), bot.getLogic().getUser(userId.toString()).getUsername(), receiver, message));
